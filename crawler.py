...
def process_courselink(link: str) -> Course:
    ...
    kind = ''.join(
        next(filter(lambda li: li.startswith('Lehrform'), lis)).split()[1:])
    ects = int(
        next(filter(lambda li: li.startswith('ECTS'), lis)).split()[-1])
    courses_of_study = [cos.string.strip()
                        for cos in soup.find_all(class_='tx_dscclipclap_header')]
    cos_modules = [cos.find_all('li') for cos in soup.find_all(
        class_='tx_dscclipclap_content')]
    modules = []
